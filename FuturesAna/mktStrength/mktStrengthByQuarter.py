...
for sub in slist:
    ...
    #newDf[2:] = newDF[2:].rolling(20).mean()
    for col in sub:
        DF_O[col] = talib.TRANGE(DF_H[col],DF_L[col],DF_C[col])
    ...
    bytr  = DF_O.resample('Q')
    opt_max = bymonth.max()
    opt_min = bymonth.min()
    opt_mean = bytr.mean()
    
    opt_range_atr_ration = (opt_max-opt_min)/opt_mean
    print(opt_range_atr_ration)
    optpre = "QtrRange"
    optname = ""
    if jjj == 0:
    ...
    opt_range_atr_ration.to_csv(optname)
    jjj += 1
